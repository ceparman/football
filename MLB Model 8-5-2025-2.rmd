...
all_games<-rbind(read_csv("new_data/Game_Results.20-24.csv"),
                 read_csv("new_data/Game_Results.12-19.csv")
)
```
...
Rolling5_Tm_Runs$Pythag5delta<-Rolling5_Tm_Runs$Tm5DayPythag - Rolling5_Tm_Runs$Opp5Pythagok 
```

```{r Remove Unncessary Variables and Convert all to Numeric}
unnecessary_vars_odds<-c("Gm","Result","Record","Opponent","GB","Win","Loss","Time","D.N","Streak", "Date_1","Date_2_1_1","Date_2_1_2","wins", "loss", "TmID", "OppID", "TmUnobsSkillLvl", "OppUnobsSkillLvl", "RollingTmUnObs", "RollingOppUnObs", "R", "RA", "Tm", "Opp")
...
hm<- prelim_model_season_and_5_data[prelim_model_season_and_5_data$H_A ==1, ] |> rename(Tm_rank = Rank)
away<-prelim_model_season_and_5_data[prelim_model_season_and_5_data$H_A ==0, ]  |> rename(Opp_rank = Rank)
...
```{r create test and train}
  ...
  library(caret)
    set.seed(123)
     ...
     train_indices <- createDataPartition(model_data$TmWin, p = 0.7, list = FALSE)
    train_df <- model_data[train_indices, ]
    test_df <- model_data[-train_indices, ]

x_train <-   train_df %>% select(-c(("TmWin")))
y_train <-   train_df %>%  pull ("TmWin")

x_test <-   test_df %>% select(-c(("TmWin")))
y_test <-   test_df %>% pull ("TmWin")
...
shap_long3<-shap.prep(xgb_model = xgb3, X_train = as.matrix(xgb_train), top_n = 20)
...
r <- data.frame( prob = xgbpred_prob3,
                 TmWin = y_test)
r <- r |>  mutate(HmProb = cut(prob,seq(0,1,.1),right = FALSE)) |>
           group_by(HmProb) |> summarise( hmWin = mean(TmWin)) |>
           mutate(viswin = 1 - hmWin) |> mutate(ratio = hmWin/viswin) |>
             ungroup()


#ggplot(r, aes(HmProb,viswin))  + geom_col() + geom_col( aes(HmProb,hmWin))


r2 <- melt(r |> select(-ratio), id.vars='HmProb')

ggplot(r2, aes(x=HmProb, y= value, , fill=variable)) +
